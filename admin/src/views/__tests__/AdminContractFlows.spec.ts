...
const editorUser = {
  ...
  is_superuser: false,
  roles: ['editor'],
  assigned_permissions: ['content.entries.read', 'content.entries.write'],
  effective_permissions: ['content.entries.read', 'content.entries.write'],
...
function tokenPayload(permissions: string[], isSuperuser = false): TokenResponse {
  return {
    ...
    user: {
      ...
      assigned_permissions: isSuperuser ? [] : permissions,
